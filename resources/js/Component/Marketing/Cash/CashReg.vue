<template>
    <Layout>
        <div
            class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-10 content-between w-full"
        >
            <!-- Left Side: Cash on Hand and Remittance -->
            <div
                class="border border-gray-300 text-black overflow-hidden flex flex-col items-center w-full h-90"
            >
                <div class="p-4 flex flex-col items-center w-full">
                    ...
                    <div class="mt-1 w-full">
                        <div
                            class="flex flex-col items-center bg-white border border-green-400 sm:rounded-lg w-full py-4 mb-4 mt-4"
                        >
            ...
            <div
                class="border border-gray-300 text-black overflow-hidden flex flex-col items-center w-full"
            >
                <div class="p-4 flex flex-col items-center w-full">
                    <div class="bg--50 w-full">
                        <div class="">
                            <label class="text-xl font-semibold"
                                >Remittance Logs</label
                            >
                        </div>
                    </div>
                    <div class="mt-1 w-full overflow-y-auto">
                        <!-- Add overflow-y-auto to enable vertical scrolling if needed -->
        ...
        <div
            class="mt-4 border border-gray-300 text-black overflow-hidden flex flex-col items-center w-full flex-grow"
        >
            <div class="p-4 flex flex-col items-center w-full">
                <div class="bg--50 w-full">
                    <div class="">
                ...
                <div class="mt-1 w-full">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr
                                v-for="(cashlog, id) in paginatedCashlogs"
                                :key="id"
                            >
                                <!-- <td
                                    class="px-6 py-4 whitespace-nowrap text-center"
...
export default {
    ...
    computed: {
        ...
        paginatedCashlogs() {
            const startIndex =
                (this.cashlogsPagination.currentPage - 1) * this.itemsPerPage;
            const endIndex = startIndex + this.itemsPerPage;
    ...
    methods: {
        ...
        getCashLogs() {
            axios.post("/get-cashlogs").then(({ data }) => {
                this.cashlogs = data;
                this.cashlogsPagination.lastPage = Math.ceil(
                    this.cashlogs.length / this.itemsPerPage
                );
            });
        },
        ...
        getCashadmins() {
            axios.post("/get-admins").then(({ data }) => {
                ...
                this.pagination.lastPage = Math.ceil(
                    this.cashadmins.length / this.itemsPerPage
                );
            });
        },
        ...
        nextCashlogsPage() {
            if (
                this.cashlogsPagination.currentPage <
                this.cashlogsPagination.lastPage
            ) {
                this.cashlogsPagination.currentPage++;
            }
        },
